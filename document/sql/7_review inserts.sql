-- 외래 키 제약 조건을 참조하는 테이블부터 순차적으로 삭제합니다.

-- 1. `review_image` 테이블 초기화
DELETE FROM review_image;

-- 2. `review` 테이블 초기화
DELETE FROM review;

-- 3. `order_item` 테이블 초기화
DELETE FROM order_item;

-- 4. `order` 테이블 초기화
DELETE FROM `order`;

-- 5. `product_option` 테이블 초기화
DELETE FROM product_option;

-- 6. `product` 테이블 초기화
DELETE FROM product;

-- 7. `product_category` 테이블 초기화
-- 1. `level` 2에 해당하는 자식 카테고리 데이터 먼저 삭제
DELETE FROM product_category WHERE level = 2;

-- 2. `level` 1에 해당하는 부모 카테고리 데이터 삭제
DELETE FROM product_category WHERE level = 1;
...
-- 11. `common_code` 테이블 초기화
DELETE FROM common_code;

-- 12. `common_code_group` 테이블 초기화
